...
namespace demonware
{
	bdStorage::bdStorage() : service(10, "bdStorage")
	{
		...
		this->register_task(25, &bdStorage::getPublisherFiles);
		...
		this->register_task(28, &bdStorage::unk);
		...
		this->map_publisher_resource("publisher_storage_surveys.csv", DW_SURVEYS);
	}
	...
	void bdStorage::getPublisherFiles(service_server* server, byte_buffer* buffer)
	{
		std::string context;
		...
		for (const auto& name : filenames)
		{
			...
			std::string filedata;
			if (this->load_publisher_resource(name, filedata))
			{
				entry->filedata = std::move(filedata);
				console::demonware("[DW]: [bdStorage]: get publisher file: %s\n", name.data());
			}
			else
			{
				entry->errorcode = game::demonware::BD_NO_FILE;
				console::demonware("[DW]: [bdStorage]: missing publisher file: %s\n", name.data());
			}
	...
	void bdStorage::unk(service_server* server, byte_buffer* buffer) const
	{
		// TODO:
		auto reply = server->create_reply(this->task_id());
		reply.send();
