...
namespace utils::cryptography
{
	...
	std::string ecc::sign_message(const key& key, const std::string& message)
	{
		...
		opts.type = LTC_ECCSIG_ANSIX962;
		opts.prng = prng_.get_state();
		opts.wprng = prng_.get_id();

		if (ecc_sign_hash_v2(cs(hash.data()), ul(hash.size()),
			buffer, &length, &opts, &key.get()) != CRYPT_OK)
		{
			return {};
		}
	...
	bool ecc::verify_message(const key& key, const std::string& message, const std::string& signature)
	{
		...
		auto result = 0;
		return (ecc_verify_hash_v2(cs(signature.data()),
			                      ul(signature.size()),
			                      cs(hash.data()),
			                      ul(hash.size()),
			                      &opts,
	...
	std::string rsa::encrypt(const std::string& data, const std::string& hash, const std::string& key)
	{
		...
		auto crypt = [&]()
		{
			...
			params.u.crypt.lparam = cs(hash.data());
			params.u.crypt.lparamlen = ul(hash.size());
			params.prng = prng_.get_state();
			params.wprng = prng_.get_id();
			params.params.hash_idx = find_hash("sha512");
			params.params.mgf1_hash_idx = find_hash("sha512");
			params.padding = LTC_PKCS_1_OAEP;

			return rsa_encrypt_key_v2(cs(data.data()), ul(data.size()),
				cs(out_data.data()), &out_len, &params, &new_key);
		};
